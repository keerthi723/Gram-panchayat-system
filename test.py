...
def pdo_add_family_member():
    ...
    if request.method == 'POST':
        ...
        qq = "insert into family_members_details values('" + str(maxin) + "','" + str(family_head_name) + "','" + str(
            family_head_aadhar_no) + "','" + str(
            ration_card_no) + "','" + str(family_member_count) + "','" + str(family_members_name) + "','" + str(door_no) + "','" + str(
            street) + "','" + str(area) + "','" + str(village) + "','" + str(post) + "','" + str(taluk) + "','" + str(district) + "','0','0','"+str(area)+"')"
...
@app.route("/pdo_home")
def pdo_home():
    return render_template('pdo_home.html')
...
def user_home():
    ...
    return render_template("user_home.html", news_items=news_items)
...
def pdo_family_details_edit1():
    ...
    if request.method == 'POST':
        ...
        taluk = request.form['taluk']
        district = request.form['district']
        qq = "update  family_members_details set family_head_name='" + str(family_head_name) + "',family_head_aadhar_no='" + str(family_head_aadhar_no) + "',ration_card_no='" + str(
                ration_card_no) + "',family_member_count='" + str(family_member_count) \
             + "',family_members_name='" + str(family_members_name) \
             + "',door_no='" + str(door_no) \
             + "',street='" + str(street) \
             + "',area='" + str(area) \
             + "',village='" + str(village) \
             ...
             + "', where id='"+str(members_eligible_for_vote)\
             + "' where id='"+str(id)+"'"
        print(qq)
        result = mm.insert_query(qq)
...
def pdo_view_suggestions():
    ...
    return render_template("pdo_view_suggestions.html", suggestions=suggestions)
...
def admin_edit_news(id):
    ...
    news_item = mm.select_direct_query("SELECT * FROM news WHERE id=%s", (id,))[0]
...
def vote():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password']
        user_check = mm.select_direct_query2("SELECT * FROM user_details WHERE username=%s AND password=%s", (aadhaar, password))
...
def submit_vote():
    ...
    mm.insert_query("INSERT INTO voting_votes (user_aadhaar, candidate_id) VALUES (%s, %s)", (aadhaar, candidate_id))
    mm.insert_query("UPDATE voting_candidates SET votes = votes + 1 WHERE id = %s", (candidate_id,))
    mm.insert_query("UPDATE family_members_details SET votes_cast = votes_cast + 1 WHERE family_head_aadhar_no = %s", (aadhaar,))
...
def vote():
    ...
    if request.method == 'POST':
        aadhaar = request.form['aadhaar']
        password = request.form['password']
        aadhar_check = mm.select_direct_query2("SELECT * FROM family_members_details WHERE family_head_aadhar_no=%s", (aadhaar)) and mm.select_direct_query2("SELECT * FROM user_details WHERE  password=%s", (password))
        if  aadhar_check :
            flash("Invalid Aadhaar or Password!", "danger")
            return redirect(url_for('vote'))
